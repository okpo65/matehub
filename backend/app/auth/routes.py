...
from app.auth.kakao import kakao_oauth
...
async def kakao_callback(
    ...
    try:
        ...
        logger.info(f"카카오 사용자 정보: kakao_id={kakao_id}")
        ...
        if not user:
            ...
            user = User(
                name=f"kakao_user_{kakao_id}",  # name 필드는 필수이므로 기본값 설정
                kakao_id=kakao_id,
        ...
        logger.info(f"카카오 로그인 완료: kakao_id={kakao_id}")
        ...
        return JSONResponse(
            ...
            content={
                ...
                "kakao_id": kakao_id,
                "user_id": user.id,  # DB의 실제 user ID
...
@router.get("/user/{kakao_id}")
async def get_user_info(kakao_id: str, db: Session = Depends(get_db)):
    """사용자 정보 조회 - kakao_id로 조회"""
    user = db.query(User).filter(User.kakao_id == kakao_id).first()
    if not user:
...
async def refresh_kakao_token(kakao_id: str, db: Session = Depends(get_db)):
    """카카오 토큰 갱신"""
    user = db.query(User).filter(User.kakao_id == kakao_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="사용자를 찾을 수 없습니다")
...
@router.post("/kakao/logout/{kakao_id}")
async def kakao_logout(kakao_id: str, db: Session = Depends(get_db)):
    """카카오 로그아웃 (토큰 무효화)"""
    user = db.query(User).filter(User.kakao_id == kakao_id).first()
    if not user:
        raise HTTPException(status_code=404, detail="사용자를 찾을 수 없습니다")
